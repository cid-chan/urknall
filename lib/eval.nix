          ...
          urknall.build.evaluator =
            ...
            let
              ...
              nix-command = toString (localPkgs.writeShellScript "evaluator" ''
                ...
                nix build "path:''${TEMP_FLAKE}#urknall-resolve.${localPkgs.system}.default.${operation}" --no-update-lock-file --log-format internal-json --json --no-link -v "$@" | ${localPkgs.jq}/bin/jq -r '"\(.[0].outputs.out)"'
